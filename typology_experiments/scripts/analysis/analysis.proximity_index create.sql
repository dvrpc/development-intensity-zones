...
with 
	proximity_index_level_column as (
		...
		select block_group20, 'very low' as proximity_index_level from analysis.proximity_index_step1 where proximity_index < (select proximity_index_thresholds from _resources.thresholds where levels = 'low') union
		select block_group20, 'low' as proximity_index_level from analysis.proximity_index_step1 where proximity_index >= (select proximity_index_thresholds from _resources.thresholds where levels = 'low') and proximity_index < (select proximity_index_thresholds from _resources.thresholds where levels = 'moderate') union
		select block_group20, 'moderate' as proximity_index_level from analysis.proximity_index_step1 where proximity_index >= (select proximity_index_thresholds from _resources.thresholds where levels = 'moderate') and proximity_index < (select proximity_index_thresholds from _resources.thresholds where levels = 'high') union
    ...
    from analysis.proximity_index_step1 b
