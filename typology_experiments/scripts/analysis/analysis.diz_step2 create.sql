drop materialized view if exists analysis.diz;

create materialized view analysis.diz as /*Found out how to create a materialized view, and what it is from https://www.postgresql.org/docs/current/rules-materializedviews.html */
with 
	diz_step1 as (
	...
	diz_average_comm_stories_column as (
	...
	diz_with_average_comm_stories_column as (
        select
            b.block_group20, 
            b.density_index, 
            b.proximity_index, 
			...
			b.geom
        from diz_step1 b
        	left join diz_average_comm_stories_column d
	...
	diz_with_crosswalk_bonus_and_stories_bonus_columns_too_step1 as (
        select
            b.block_group20, 
            b.density_index, 
            b.proximity_index, 
			...
			b.geom
        from diz_with_average_comm_stories_column b
        	left join crosswalk_density_summary d
            on b.prelim_diz_zone_plus_1 = d.prelim_diz_zone_plus_1
    	),
	diz_with_crosswalk_bonus_and_stories_bonus_columns_too as (
		...
		from diz_with_crosswalk_bonus_and_stories_bonus_columns_too_step1
	
		),
	diz_with_diz_zone_column_too as (
		...
		from diz_with_crosswalk_bonus_and_stories_bonus_columns_too
	...
	diz as (
        select
            ...
            d.diz_zone_name,
			b.geom
        from diz_with_diz_zone_column_too b
    ...
    select row_number() over() as row_number, block_group20, density_index, proximity_index, density_index_level, proximity_index_level, prelim_diz_zone, crosswalk_density, average_comm_stories, crosswalk_bonus, stories_bonus, diz_zone, diz_zone_name, geom from diz