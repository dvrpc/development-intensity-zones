...
with 
	density_and_accessibility_table_without_crosswalk_density_columns as (select * from analysis.bones_density_and_accessibility),
	...
	bones_test_results_density_level_column as (
		...
		select block_group20, density, accessibility, density_2050, accessibility_2050, crosswalk_aland_density, crosswalk_non_pos_water_density, 'very low' as density_level from density_and_accessibility_table where density < (select density_thresholds from thresholds where levels = 'low') union
		select block_group20, density, accessibility, density_2050, accessibility_2050, crosswalk_aland_density, crosswalk_non_pos_water_density, 'low' as density_level from density_and_accessibility_table where density >= (select density_thresholds from thresholds where levels = 'low') and density < (select density_thresholds from thresholds where levels = 'moderate') union
		select block_group20, density, accessibility, density_2050, accessibility_2050, crosswalk_aland_density, crosswalk_non_pos_water_density, 'moderate' as density_level from density_and_accessibility_table where density >= (select density_thresholds from thresholds where levels = 'moderate') and density < (select density_thresholds from thresholds where levels = 'high') union
		select block_group20, density, accessibility, density_2050, accessibility_2050, crosswalk_aland_density, crosswalk_non_pos_water_density, 'high' as density_level from density_and_accessibility_table where density >= (select density_thresholds from thresholds where levels = 'high') and density < (select density_thresholds from thresholds where levels = 'very high') union
		select block_group20, density, accessibility, density_2050, accessibility_2050, crosswalk_aland_density, crosswalk_non_pos_water_density, 'very high' as density_level from density_and_accessibility_table where density >= (select density_thresholds from thresholds where levels = 'very high') and density < (select density_thresholds from thresholds where levels = 'extreme') union
		select block_group20, density, accessibility, density_2050, accessibility_2050, crosswalk_aland_density, crosswalk_non_pos_water_density, 'extreme' as density_level from density_and_accessibility_table where density >= (select density_thresholds from thresholds where levels = 'extreme') union
		select block_group20, density, accessibility, density_2050, accessibility_2050, crosswalk_aland_density, crosswalk_non_pos_water_density, 'low' as density_level from density_and_accessibility_table where density is null
		
		), /*Note that the density_2050, accessibility_2050, crosswalk_aland_density, and crosswalk_non_pos_water_density columns are brought up here only so that they can be carried over throughout the rest of the script, no operations are done on or related to those columns here. Also, basically figured out how to retrieve a cell value using SQL from https://stackoverflow.com/a/56322459 (in turn found on https://stackoverflow.com/questions/56322358/postgresql-how-to-copy-the-value-of-a-cell-in-a-row-and-paste-it-into-another-c )*/
    ...
    select row_number() over() as row_number, block_group20, density, accessibility, density_level, accessibility_level, area_type, density_2050, accessibility_2050, crosswalk_aland_density, crosswalk_non_pos_water_density, density_level_2050, accessibility_level_2050, area_type_2050, geom from bones_test_results_before_unique_id