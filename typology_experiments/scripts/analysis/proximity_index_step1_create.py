...
data_for_proximity_index_column = pd.merge(
    ...
    on=["GEOID"],
    how="left",
)  # Left joins data_for_tot_gq_hu_5mi_column to data_for_tot_comm_sqft_thou_2mi_column to essentially start getting the data for the eventual proximity_index columns
...
data_for_proximity_index_column["proximity_index"] = (
    ...
    * 2
    ...
    data_for_proximity_index_column["tot_comm_sqft_thou_2mi"]
    + data_for_proximity_index_column["tot_gq_hu_5mi"]
)  # Creates the actual eventual proximity_index column

data_for_proximity_index_column = data_for_proximity_index_column[
    ["GEOID", "proximity_index"]
]  # Just keeps the columns I want to keep and in the order I want to keep them in

block_groups_24co_2020 = block_groups_24co_2020.merge(
    data_for_proximity_index_column, on=["GEOID"], how="left"
)  # Left joins data_for_proximity_index_column to block_groups_24co_2020


proximity_index_step1 = pd.DataFrame(block_groups_24co_2020[["GEOID", "proximity_index"]]).rename(
